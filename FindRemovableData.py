...
def main(path, savePath):
    ...
    def second_find_instrument_model_cell(currentSheet):
        for row in range(1, 30):
            for column in "GHIJKLMNOPQRSTUVWXYZ":  # Here you can add or reduce the columns
                ...
                if (modelVal is None) or isinstance(modelVal, float) is True:
                    continue
        ...
        for row in range(1, 30):
            for column in "GHIJKLMNOPQRSTUVWXYZ":  # Here you can add or reduce the columns
                ...
                if (modelVal[:6] == "ASC-DP") or (modelVal[:4] == "2929") or (modelVal[:4] == "3030"):
                    ...
                    elif currentSheet[gammaRow + str(bettarow + 2)].value is not None:
                        return [row, column, modelCell, modelVal]
                    else:
    ...
    def find_survey_number(currentSheet):
        for column in "ABCDEFGHI":
            for row in range(1, 20):
                ...
                while (currentSheet[modelCell].value == "Survey No") or (
                        currentSheet[modelCell].value == "Survey Number"):
                    if newCol >= "V":
                        continue
    ...
    for file in files:
        ...
        for x in allSheetNames:
            ...
            # This algorithm can only handle files that have more than 3 Betta-Gamma instances
            # This will let us check for that and run it in a later part
            ...
            if betaRow is None or betaCol is None:
                if invalidSheets.count(file) > 0:
                    ...
                    print("adding ", file, " to the invalid")
                    needToContinue = True

            # If there is less than 3 instances of Beta-Gamma then skip it
            if needToContinue:
                continue
    ...
    for file in invalidSheets:
        ...
        for x in allSheetNames:
            ...
            if currentSheet[betaCol + str(betaRow + 2)].value is None:
            ...
            # If there was no data found then don't add to the list
            if good:
                # Write the current Worksheet
                QCworksheet.write(QCfileRow, 0, tail)
                QCworksheet.write(QCfileRow, 1, currentSheetString)
